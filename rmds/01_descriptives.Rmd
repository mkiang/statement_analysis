...
Here, we present analytic code for our paper, "Analytic code and additional analyses for 
  *Statements issued by academic medical institutions after George Floyd’s 
  killing by police and subsequent unrest in the United States: 
  cross-sectional study". By default, the code is hidden — to view the code, simply click on the "Code" button to the right of each chunk. We used *`r R.version$version.string`* for all analyses and full session information can be viewed at the end of the document. When possible, we save the de-identified intermediate data frames in `csv` format. 
